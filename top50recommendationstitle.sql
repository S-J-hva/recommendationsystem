...
## update the dataset with title as vector from lexemestitle
...
WHERE lexemestitle @@ to_tsquery('fast+furious')
...
SELECT title FROM movies WHERE url='2-fast-2-furious')));
...
CREATE TABLE recommendationsBasedOnTitleField AS SELECT url,
...
\copy (SELECT * FROM recommendationsBasedOnTitleField) to
'/home/pi/RSL/top50recommendationstitle.csv' WITH csv 
